...
export default class UserControllerOnion implements IUserControllerOnion /* TODO: extends ../core/infra/BaseController */ {
    constructor(
        ...
        @Inject(config.controllers.role.name) private roleControllerInstance: IRoleService
    ) {
    }

    public async getUsersByRoleId(req: Request<ParamsDictionary, any, any, ParsedQs, Record<string, any>>, res: Response<any, Record<string, any>>, next: NextFunction) {
        try {
            const roleId = req.params.id;
            const users = await this.userServiceInstance.getUsersByRole(roleId) as Result<Array<IUserDTO>>;
            if (users.isFailure) {
