...
export default class UserService implements IUserService {
    ...
    public async SignUpPatient(userDTO: IPatientDTO): Promise<Result<{ userDTO: IPatientDTO, token: string }>> {
        try {
            ...
            const roleOrError = await this.getRole(userDTO.role);
            if (roleOrError.isFailure) {
                return Result.fail<{ userDTO: IPatientDTO; token: string }>(roleOrError.error);
            } else {
                role = roleOrError.getValue();
            }

            const userOrError = await Patient.create({
                firstName: userDTO.firstName,
                lastName: userDTO.lastName,
                email: email,
                role: role,
            ...
            await this.userRepo.savePatient(userResult);
            const userDTOResult = PatientMap.toDTO(userResult) as IPatientDTO;
            return Result.ok<{ userDTO: IPatientDTO, token: string }>({userDTO: userDTOResult, token: token})
    ...
    public async SignInPatient(email: string, password: string): Promise<Result<{ userDTO: IPatientDTO, token: string }>> {

        const user = await this.userRepo.findPatientByEmail(email);
        ...
        if (validPassword) {
            ...
            const userDTO = PatientMap.toDTO(user) as IPatientDTO;
            return Result.ok<{ userDTO: IPatientDTO, token: string }>({userDTO: userDTO, token: token});
        } else {
            throw new Error('Invalid Password');
        }
    }
